...
class BookAppointment(LoginRequiredMixin, CreateView):
    '''
    Once the booking has been created, the user will be directed to the \
    manage booking page.
...
class ManageAppointment(generic.ListView):
    '''
    The Manage page will allow the user to view the booking,
...
class DeleteAppointment(DeleteView):
    '''
    The delete page will allow the user to delete the booking,
    if they no longer require the appointment.
    '''
...
class UpdateAppointment(LoginRequiredMixin, UpdateView):
    '''
    The user will have a option to update the booking and view
    the updated changes.
    '''
    model = BookAppointmentModel
    ...
    def get(self, request, *args, **kwargs):
        ...
        if appointment.patient != request.user:
            # The user will be restricted to view other users booking,
            # prompting a error
            messages.error(request, "You do not have access to manage \
            this booking.")
            return redirect('manage-appointments')
        else:
